...
def countX(strr):
  if(len(strr)==1 and strr != "x" or len(strr)==0):
    return 0 
  if(strr=="x"):
    return 1
  if(strr[-1]=="x"):
...
def countHi(strr):
  if(len(strr) == 2 and strr != "hi" or len(strr) == 0):
    return 0
  if(strr == "hi"):
    return 1
  if(strr[-2:] == "hi"):
    return 1+countHi(strr[:-1])
...
def changeXY(strr):
  newstrr=""
  if(len(strr)==0 or len(strr) == 1 and strr != "x"):
    return newstrr
  if(strr[-1] == "x"):
    return "y"+newstrr+changeXY(strr[:-1])
  return strr[-1]+newstrr+changeXY(strr[:-1])
